...
- Reconciliation remains device-side. The Worker cannot access a Git working tree or Git credential. While Background Sync is enabled, the user controls automatic pull and automatic publishing independently; publishing remains default-off. Those preferences and every pull/stage/commit/push operation remain local to the installation and are never added to the relay API or schema.
