...
final class GitLFSCitadelSSHAuthenticator: GitLFSSSHAuthenticator {
    ...
    func authenticate(request: GitLFSSSHAuthRequest, credentials: GitRemoteCredentials) async throws -> GitLFSAccess {
        ...
        return try await Self.withEventLoopGroup { group in
    ...
    private static func withEventLoopGroup<Result>(
        _ operation: (MultiThreadedEventLoopGroup) async throws -> Result
    ) async throws -> Result {
        let group = MultiThreadedEventLoopGroup(numberOfThreads: 1)
        do {
            let result = try await operation(group)
