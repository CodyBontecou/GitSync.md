...
async function main() {
  ...
  const plan = applyScreenOverrides(buildScreenshotPlan(insights, options), options);
  ...
  if (!process.env.OPENAI_API_KEY) {
    throw new Error(
      "--generate was requested, but OPENAI_API_KEY is missing. Add it to .env or export it in your shell before running generation.",
    );
  }
...
function applyScreenOverrides(plan: ScreenPlan[], options: CliOptions): ScreenPlan[] {
  const overridesPath = path.join(repoRoot, "app-store-input/screens.json");
  if (!fs.existsSync(overridesPath)) return plan;

  const overrides = JSON.parse(fs.readFileSync(overridesPath, "utf8")) as ScreenOverride[];
  if (!Array.isArray(overrides)) {
    throw new Error(`${relativePath(overridesPath)} must be a JSON array of screen overrides.`);
  }

  return plan.map((screen, index) => {
    const override = overrides[index];
    if (!override) return screen;

    const next: ScreenPlan = { ...screen };
    if (typeof override.title === "string" && override.title.trim()) next.title = override.title.trim();
    if (typeof override.feature === "string" && override.feature.trim()) next.feature = override.feature.trim();
    if (typeof override.headline === "string" && override.headline.trim()) next.headline = trimCopy(override.headline.trim(), 64);
    if (typeof override.subheadline === "string" && override.subheadline.trim()) {
      next.subheadline = trimCopy(override.subheadline.trim(), 104);
    }
    if (typeof override.mood === "string" && override.mood.trim()) next.mood = override.mood.trim();
    if (typeof override.prompt === "string" && override.prompt.trim()) next.prompt = override.prompt.trim();

    const screenshotOverride = typeof override.screenshotPath === "string" ? override.screenshotPath : override.screenshot;
    if (typeof screenshotOverride === "string" && screenshotOverride.trim()) {
      const resolvedScreenshot = resolveRepoPath(screenshotOverride.trim());
      const exists = fs.existsSync(resolvedScreenshot);
      next.screenshotPath = exists ? resolvedScreenshot : null;
      next.screenshotSource = exists
        ? relativePath(resolvedScreenshot)
        : `missing override screenshot: ${screenshotOverride.trim()}`;
      next.draftOnly = !exists;
    }

    const overrideSlug = typeof override.slug === "string" && override.slug.trim() ? override.slug.trim() : `${index + 1}-${next.headline}`;
    next.slug = slugify(overrideSlug);
    next.outputBackgroundPath = path.join(options.outputDir, "backgrounds", `${next.slug}-${options.targetSizeKey}-background.png`);
    next.outputFinalPath = path.join(options.outputDir, "final", `${next.slug}-${options.targetSizeKey}.png`);
