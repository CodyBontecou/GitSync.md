...
- Info.plist `UIBackgroundModes = ["fetch", "processing", "remote-notification"]` and permits `com.bontecou.Sync-md.background-refresh` (BGAppRefreshTask — the primary closed-app freshness opportunity) plus `com.bontecou.Sync-md.background-sync` (BGProcessingTask fallback with longer runtime, network required, external power not required). Both use a 15-minute `earliestBeginDate` and remain discretionary—the date is not a guaranteed interval or real-time execution claim. Foreground activation is another reconciliation trigger. When separately opted-in Push Sync delivers a valid combined alert/background APNs payload, a 25-second bridge can request targeted reconciliation of the matching GitHub repository and configured branch; that wake is also discretionary. Rapid scene bounces coalesce into a running foreground pass and a 30-second completion cooldown suppresses redundant passes; an explicit `reconcileNow()` ("Sync now" / "Retry") bypasses the cooldown.
...
- Eligibility: every non-excluded cloned repository is included locally; no GitHub App linkage or server-side enrollment controls inclusion. A separately registered Push Sync repository/branch hint can target only that same locally included set.
- Triggers: scene activation, primary best-effort `BGAppRefreshTask`, fallback best-effort `BGProcessingTask`, and an optional separately consented APNs wake. Production explicitly injects `SystemPremiumBackgroundProcessingScheduler` into `PremiumRuntime`; the initializer has no scheduler default, while tests may explicitly inject a no-op or recording double. Both request types are rescheduled at invocation, retained through exactly-once completion, and expiration completes false while cancelling processing flights. Foreground work is serialized one repository at a time; processing may batch up to three.
...
- `@UIApplicationDelegateAdaptor(SyncAppDelegate.self)` handles APNs registration, notification routing, and the optional 25-second Push Sync bridge into targeted Background Sync reconciliation. It does not gate or prove scheduled Background Sync.
...
- `SyncAppDelegate` belongs to separately inventoried Push Sync. Its APNs bridge may invoke the Background Sync engine, but must not be presented as BGTask scheduling evidence or a guaranteed OS wake.
...
- **Resolved:** Background Sync uses primary BGAppRefresh plus processing fallback and foreground reconciliation. Separately opted-in Push Sync can accelerate it through an alert-plus-background APNs request; the removed subscription-era premium relay does not participate.