...
- Triggers: (a) silent APNs push → `PremiumSilentPush.parse(userInfo)` with `channel` + `hintID`; gated on `entitlementIsActive()` and a repo with `assist.enabled && assist.channel == push.channel`; duplicate hints deduped via 256-entry LRU. (b) Foreground `reconcileForeground()` over all assist-enabled repos — called from app `.task` and `scenePhase == .active` (with `premiumRuntime.start()`).
- Per-repo policies (`RepoAssistSettings`): `networkPolicy == .wifiOnly` (NWPathMonitor `SystemBackgroundSyncConditions`), `powerPolicy == .externalPowerOnly` (batteryState charging/full), `selectedBranch` override else repo.branch. Violations → `.deferred("Waiting for Wi-Fi."/"Waiting for external power.")` recorded as health `.deferred`.
