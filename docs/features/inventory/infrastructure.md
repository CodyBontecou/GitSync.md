...
Best-effort GitHub push wake-hints → silent APNs. GitHub App `push` webhook (raw-body HMAC-SHA256 verified, D1 dedupe + transactional outbox → Queue → APNs `content-available`-only payload with opaque channel/event hint — no repo/branch/path/credential data). iOS endpoints: `PUT /v1/entitlements` (StoreKit-verified installation credential + short-lived bearer + deletion capability), `PUT /v1/devices` / `DELETE /v1/devices` (APNs token lifecycle with monotonic registration generation), `DELETE /v1/installation` (idempotent full purge via hashed deletion receipts). Relay setup: `POST /v1/github/link/start`, `GET /v1/github/callback` (hashed single-use state), `GET /v1/github/link/status`, `POST|DELETE /v1/enrollments[/:channel]`, `POST /v1/webhooks/github`, `POST /v1/app-store/notifications`. Secrets: GitHub App private key, webhook secret, APNs key. `KILL_SWITCH` var halts admissions. Daily cron GC. Requires StoreKit `appAccountToken` == installation UUID binding (single-device entitlement per purchase in v1).
