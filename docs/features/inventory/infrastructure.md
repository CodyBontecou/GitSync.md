...
## Gaps / uncertainties
- **OAuth security gaps (potential findings):** no PKCE and, critically, the callback never validates `state` — CSRF on the OAuth flow is possible in principle; fallback state uses weak `Math.random()`. The access token is delivered via redirect URL (standard for ASWebAuthenticationSession but worth noting). The dormant paid-unlock Worker's `verify-legacy-jws` endpoint decodes its JWS payload without signature verification; do not conflate that legacy receipt path with Background Sync. `PremiumStorefront.swift` and the former premium relay's server verifier were removed, and Background Sync performs no StoreKit entitlement verification.
