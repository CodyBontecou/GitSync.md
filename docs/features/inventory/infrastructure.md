...
3. **premium-workers.yml** — push/PR touching `worker/**`. Matrix over premium-relay, storekit-verifier (with startup check), onboarding-analytics: typecheck/tests/migrations validation per worker. Gate for infra code.
...
### b) `gitsync-premium-relay` (`worker/premium-relay/`, `wrangler.jsonc`: D1 `premium-relay`, Queue `premium-relay-outbox`, service binding `STOREKIT_VERIFIER`)
Best-effort GitHub push wake-hints → silent APNs. The app API never sends repository names/URLs/content/local paths/Git credentials. Signed GitHub App `push` webhook bodies are raw-body HMAC-SHA256 verified and may transiently contain names/URLs/commit messages/paths/authors; the relay never logs or persists those descriptive fields, extracting only numeric repository ID and branch into D1 dedupe + transactional outbox records. Queue delivery uses an APNs `content-available`-only payload with opaque channel/event hint. iOS endpoints: `PUT /v1/entitlements` (StoreKit-verified installation credential + short-lived bearer + deletion capability), `PUT /v1/devices` / `DELETE /v1/devices` (APNs token lifecycle with monotonic registration generation), `DELETE /v1/installation` (terminal tombstone/removal with hashed deletion receipts and retention-scoped operational records). Relay setup uses two browser legs: `POST /v1/github/link/start`; `GET /v1/github/callback` validates and rotates the hashed setup state, binds the proposed installation, and redirects without linking; `GET /v1/github/authorize/callback` uses a transient GitHub App user token to require exact personal ownership or active organization-owner authority before the final D1 link and safe `syncmd://assist-linked` HTML handoff. `GET /v1/github/link/status` and new enrollment revalidate the retained numeric administrator identity. Other routes are `POST|DELETE /v1/enrollments[/:channel]`, `POST /v1/webhooks/github`, and `POST /v1/app-store/notifications`. GitHub App permissions are Contents read-only plus Organization members read-only; secrets are the App private key, webhook secret, OAuth client secret, and APNs key. `KILL_SWITCH` halts admissions. Daily cron performs retention cleanup. Requires StoreKit `appAccountToken` == installation UUID binding (single-device entitlement per purchase in v1).

### c) `storekit-verifier` (`worker/storekit-verifier/`, no public route — service-binding only)
Verifies StoreKit2 transaction JWS and App Store Server Notification v2 JWS via Apple's official `@apple/app-store-server-library` `SignedDataVerifier`: ES256/x5c chain against pinned Apple Root CA fingerprints, purpose OIDs, bundle ID, environment, appAccountToken, expiry/revocation, online checks on by default, fail-closed.
...
## Gaps / uncertainties
- **OAuth security gaps (potential findings):** no PKCE and, critically, the callback never validates `state` — CSRF on the OAuth flow is possible in principle; fallback state uses weak `Math.random()`. The access token is delivered via redirect URL (standard for ASWebAuthenticationSession but worth noting). `verify-legacy-jws` decodes the JWS payload without signature verification on this worker — it trusts Apple's transport only if the caller is the app; the crypto verification lives in the separate storekit-verifier worker used by the premium relay, not by this legacy path.
