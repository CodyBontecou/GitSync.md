...
- **Background processing reconciliation**: production `Sync_mdApp.init` must explicitly inject `SystemPremiumBackgroundProcessingScheduler` into `PremiumRuntime`; the runtime's omitted-injection default is intentionally no-op for tests and other controlled composition. With global Background Sync enabled and either automatic action selected, the system scheduler registers two complementary best-effort opportunities: `com.bontecou.Sync-md.background-refresh` (`BGAppRefreshTask`, primary closed-app freshness opportunity) and `com.bontecou.Sync-md.background-sync` (`BGProcessingTask` fallback, longer runtime, network required, external power not required). Both use a 15-minute `earliestBeginDate`, reschedule on invocation, and cancel processing flights on expiration (`PremiumBackgroundProcessingExecution` with exactly-once completion). Fifteen minutes is not a promised interval; iOS controls whether and when they run. `prepareForSettings()` performs the settings-side refresh/reconciliation.
