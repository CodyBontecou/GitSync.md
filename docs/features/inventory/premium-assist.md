...
- Triggers: `reconcileForeground()`, `reconcileProcessing()`, and single-repo `reconcile(repoID:)`. Foreground reconciliation is deliberately serialized one repository at a time; processing may batch up to 3. Per-repo in-flight dedup uses generation-tagged flights, with independent foreground/processing cancellation sets and `cancelAll()` when either action consent is revoked.
...
- Background Sync sends nothing anywhere except normal Git traffic (fetch/push) from the app's libgit2 engine directly to the user's configured Git provider, using credentials already on the device. No relay, no push registration, no server-side Background Sync data, and no StoreKit entitlement verification. The app's StoreKit review-request API is unrelated. The separate opt-in Push Sync feature has APNs configuration and a small relay, but neither gates Background Sync.
...
- `UIBackgroundModes` contains `fetch` and `processing`, not `remote-notification`; Info.plist permits both Background Sync identifiers and has no `PREMIUM_RELAY_BASE_URL`. The app entitlement does carry `aps-environment` for the separate Push Sync notification feature—never describe it as Background Sync gating.
