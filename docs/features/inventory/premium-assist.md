...
Domain: client (Sync.md), with optional event acceleration through the separate `push-worker/`. Background Sync is part of the paid-up-front app: no subscription or entitlement verification. Scheduled reconciliation needs no server, and every Git operation runs on-device through the app's libgit2 engine. The historical premium relay (Cloudflare Worker + D1 + Queues + storekit-verifier) and StoreKit subscription tier were removed; the narrower opt-in Push Sync relay can now attach a best-effort background wake to its visible alert.
...
- **Release gate**: `PremiumRuntime` defaults its injected `assistFeatureIsEnabled` boundary to `FeatureFlags.gitSyncAssistEnabled`. When false, startup, global enable, settings reconciliation, and processing work perform no scheduler or Git work. Persisted global preferences remain intact rather than being rewritten merely because the release gate is off. Manual Git, Shortcuts, and callback paths are outside this gate. No StoreKit check exists.
...
- **Foreground reconciliation**: `reconcileForeground()` on scene-active reconciles the automatic inventory, then attempts eligible repositories through the coordinator. Rapid scene bounces coalesce into a running pass (no cancel-and-restart), a completed pass stamps a 30-second cooldown that suppresses redundant bounces, and a pass cancelled before finishing leaves the cooldown unstamped so the next activation retries immediately. Explicit in-app actions (`reconcileNow()` behind "Sync now" / "Retry") bypass the cooldown.
- **Background processing reconciliation**: production `Sync_mdApp.init` explicitly injects `SystemPremiumBackgroundProcessingScheduler` into `PremiumRuntime`. The initializer has no scheduler default; tests and other controlled compositions must explicitly inject a no-op or recording double. With global Background Sync enabled and either automatic action selected, the system scheduler registers two complementary best-effort opportunities: `com.bontecou.Sync-md.background-refresh` (`BGAppRefreshTask`, primary closed-app freshness opportunity) and `com.bontecou.Sync-md.background-sync` (`BGProcessingTask` fallback, longer runtime, network required, external power not required). Both use a 15-minute `earliestBeginDate`, reschedule on invocation, and cancel processing flights on expiration (`PremiumBackgroundProcessingExecution` with exactly-once completion). Fifteen minutes is not a promised interval; iOS controls whether and when they run. `prepareForSettings()` performs the settings-side refresh/reconciliation.
- **Push-accelerated reconciliation**: `Sync_mdApp` connects `PushSyncNotificationBridge` to the runtime. A valid combined alert/background payload names a GitHub `owner/repo`, exact branch, target SHA, and opaque hint. `processPush` requires the feature flag, global Background Sync, automatic pull, local inclusion, a cloned matching GitHub remote, and the exact configured branch. The hint never supplies Git bytes; normal authenticated fetch and safety classification remain authoritative. The bridge calls Apple's completion exactly once, times out after 25 seconds, and cancels only flight generations actually started by that push. iOS may suppress the callback; the visible alert remains the explicit fallback.
...
- Triggers: `reconcileForeground()`, `reconcileProcessing()`, `reconcilePush(repoIDs:hintID:)`, and single-repo `reconcile(repoID:)`. Foreground reconciliation is deliberately serialized one repository at a time; processing and a matching push may batch up to 3. Per-repo in-flight dedup uses generation-tagged flights, with independent foreground/processing/push ownership so APNs timeout cannot cancel pre-existing work, plus `cancelAll()` when either action consent is revoked.
...
- Scheduled Background Sync sends nothing anywhere except normal Git traffic (fetch/push) from the app's libgit2 engine directly to the user's configured Git provider, using credentials already on the device. It requires no relay and has no server-side Background Sync data or StoreKit entitlement verification. If Push Sync is independently enabled, its relay stores repository names, APNs routing, and owner-verified GitHub App installation/account identifiers, then transiently forwards branch/SHA/hint metadata in an alert; it never receives file contents, device Git credentials, or local paths. Short-lived GitHub tokens are used only for owner proof/revalidation, immediately revoked, and never stored. Push Sync acceleration does not gate scheduled Background Sync.
...
- `UIBackgroundModes` contains `fetch` and `processing` for scheduled Background Sync plus `remote-notification` for optional Push Sync acceleration. Info.plist permits both scheduler identifiers and has no `PREMIUM_RELAY_BASE_URL`. The app entitlement carries `aps-environment` for Push Sync; neither that entitlement nor the relay gates scheduled Background Sync.
