...
- **`removeRepo(id:deleteLocalFiles:)`** (AppState.swift:2847–2862): notifies `assistRepositoryRemovalHandler` if repo has a Background Sync channel; deletes vault dir only when `deleteLocalFiles && repo.isGitSyncManagedStorage`; clears custom location scope, **deletes Keychain repo credentials** (`clearRemoteCredentials`), clears caches, removes + saves.
...
- **`updateRepo(id:mutate:)`** (AppState.swift:2819–2827): generic mutation with Background Sync registration diff → `assistConfigurationChangeHandler?()`.
...
## 18. Background Sync (Premium automation) integration
