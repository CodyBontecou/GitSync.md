...
| 15.5 | Push Sync opt-in (App Settings → Push Sync, plus the per-repo Settings surface): notification authorization, APNs registration, Keychain device secret, overridable worker URL; disable unregisters device + APNs; the installation-global toggle registers every cloned GitHub repository (new clones re-register automatically) | Core | `PushSyncManager.swift:51-132`, `AppSettingsView.swift` (global surface), `SettingsView.swift:303` (per-repo surface) (inventory §7, §10) |
...
| 15.9 | APNs token-based provider auth (ES256 JWT, 30-min cache, sandbox + production hosts, passive alerts, collapse IDs); 41 vitest tests (helpers, webhook seam incl. device pruning, throttle-skip, rate limit, endpoints, pagination); Swift tests pin registration-body privacy filter + intentional `aps-environment`/background-modes config | Dev | `push-worker/src/apns.ts`, `webhook.test.ts`, `SyncMDTests.swift:951-994, 2204-2244` (inventory §14, §16) |
