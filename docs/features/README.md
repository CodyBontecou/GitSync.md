...
7. **Automation & integrations** — x-callback-url API, pull/push/sync App Intents with fail-closed publishing, explicit widget/Control Center pulls, foreground reconciliation, best-effort BGAppRefresh/BGProcessing opportunities, and optional one-time-GitHub-App Push Sync/APNs acceleration.
8. **Background Sync (included with the app)** — independently controlled fail-closed automatic pull and separately consented automatic commit/push, including safe push-only mode without checkout, local per-repo inclusion, network/power policies, health/attention surfacing, and best-effort on-device reconciliation. Scheduled operation requires no relay, subscription, or StoreKit entitlement verification; independently opted-in Push Sync can request a targeted wake.
