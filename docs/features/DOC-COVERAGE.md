...
3. ⚠️ **Site Background Sync copy needs a post-removal pass** — the 2026-08-30 subscription/relay wording was accurate for that candidate but was superseded when Background Sync became included and entirely on-device. Keep the best-effort timing qualification; remove current subscription/relay claims in an authorized site lane.
...
- Site: the once-fixed optional-subscription wording is stale again after the on-device/included architecture change — **open outside this docs-only lane** (item 3).
